...
export class CheckoutPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.cartService
      ...
      .pipe(map((pg: PaymentGateway[]) => pg.filter((p) => p.enabled === true)))
      .subscribe((pg) => {
    ...
    this.cartService.cartData.subscribe(
      (data) => (this.products = data.productData)
    );
    ...
    this.cartService
      .getTaxes()
      .pipe(map((taxes: any[]) => taxes.map((t) => t.rate)))
      .subscribe(
        (data) => {
          let taxRate: number;
          // @ts-ignore
          ...
          if (taxCalculation < 1) {
            ...
            this.finalTax = Math.floor(
              Math.abs((this.taxesRate * this.subTotal) / 100)
            );
          }
